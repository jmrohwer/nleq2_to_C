      SUBROUTINE NLEQ2E(N,X,RTOL,IERR)
C*    Begin Prologue NLEQ2E
      INTEGER N
...
C     (E)asy-to-use driver routine for NLEQ2.
C
...
C
C     (The iteration is done by subroutine N2INT actually. NLEQ2E
C      calls the standard interface driver NLEQ2, which itself does 
...
C                             If <0: NLEQ2E will be terminated with 
...
C     Checking dimensional parameter N
      IF ( N.LT.1 .OR. N.GT.NMAX ) THEN
        IF (MPRERR.GE.1)  WRITE(LUERR,1001) NMAX, N
...
C     Maximum number of Newton iterations
      IWK(31) = 100
C
      CALL NLEQ2(N,FCN,DUMMY,X,XSCAL,RTOL,IOPT,IERR,LIWK,IWK,LRWK,RWK)
      IF (IERR.EQ.82 .AND. IWK(23).LT.0) IERR=IWK(23)
C     End of subroutine NLEQ2E
      RETURN
      END
